"""
run_xai_v3_1.py -- Patient-diverse, risk-stratified XAI for PediaCare-Net v2
==========================================================
...
1) Preflight only (fast; verifies probability range and patient selection):
   python run_xai_v3_1.py --models tsl_gru --horizon 30 --check_only
...
def prepare_output(base_out: Path):
    ...
    for name in ["xai_results_v3_1.json", "case_explanations.csv"]:
        p = base_out / name
        if p.exists():
            p.unlink()
    return plots
...
def main():
    ...
    base_out = Path(a.out) if a.out else Path(config.RESULTS) / "RQ3_xai_v3_1" / run_id
    ...
    if a.check_only:
        print("\nCHECK-ONLY PASSED: probability outputs are not double-sigmoided and case selection is patient-diverse + risk-stratified.")
    ...
    doc = {
        ...
        "case_selection": "one risk-stratified representative validation window per unique subject",
    ...
    with open(base_out / "xai_results_v3_1.json", "w", encoding="utf-8") as f:
        json.dump(doc, f, indent=2, default=float)

    print(f"\nWROTE:\n  {base_out / 'xai_results_v3_1.json'}")
