"""
xai_engine_v3.py -- Temporal Integrated Gradients + constrained counterfactuals
...
MIN_RISK_DELTA = 0.005  # 0.5 percentage points
...
class DiCE:
    ...
    def generate(
        ...
        with torch.no_grad():
            ...
            for k in range(self.n_cf):
                ...
                for j, ci in enumerate(act_idx):
                    ...
                    under_p99 = bool(np.all(raw_new[changed] <= p99 + 1e-6))
                    ok = nonnegative and under_p99
                    validity[name] = {
                ...
                reduction = float(max(0.0, orig_risk - float(risks[k])))
                attempts.append(
                    {
                        "risk": float(risks[k]),
                        "reduction": reduction,
                ...
                if not changes_raw or reduction < MIN_RISK_DELTA:
                    continue
                ...
                reported.append(
                    Counterfactual(
                        ...
                        action_en=_action_text(
                            changes_raw, orig_risk, float(risks[k]), plausible
                        ),
                    )
                )
        ...
        if not reported:
            ...
            elif not best["changes_raw"]:
                note = "Optimisation did not produce a clinically reportable input change."
            else:
                note = (
                    "No candidate reduced model-predicted risk by at least "
                    f"{MIN_RISK_DELTA*100:.1f} percentage points within the reporting rules."
                )
        self.last_diagnostics = CounterfactualDiagnostics(
...
def _action_text(
    changes_raw: Dict[str, float], r0: float, r1: float, plausible: bool
) -> str:
    parts = []
    ...
    sentence = (
        ...
        f"{r0*100:.1f}% to {r1*100:.1f}% if " + " and ".join(parts) + "."
    )
    if not plausible:
        sentence += " The modified values exceed the cohort p99 bound and are flagged as implausible."
    return sentence
...
class XAIEngineV3:
    ...
    def explain(
        ...
        cfs = self.cf.generate(x, h_idx, ctx, risk, seed=cf_seed) if with_cf else []
