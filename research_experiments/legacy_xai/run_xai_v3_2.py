"""
run_xai_v3_2.py -- All-patient, decision-flipping XAI for PediaCare-Net v2
==========================================================
...
This runner fixes the v2 issues discovered during thesis validation:
  ...
  * outputs go to a new v3.2 directory, so earlier plots are not mixed in.

Recommended workflow
--------------------
1) Preflight only (fast; verifies probability range and patient selection):
   python run_xai_v3_2.py --models tsl_gru --horizon 30 --check_only

2) Small smoke run:
   python run_xai_v3_2.py --models tsl_gru --horizon 30 --n_pop 20 --n_case 3 \\
       --n_heatmaps 3 --ig_steps 8 --n_iter 30 --n_cf 4

3) Full thesis run:
   python run_xai_v3_2.py --models tsl_gru --horizon 30 --n_pop 2000 \\
...
def plot_cf_quality(results: Dict[str, List[Dict]], path):
    ...
    for m in models:
        cases = results[m]
        success.append(
            100.0 * np.mean([len(c["counterfactuals"]) > 0 for c in cases])
            if cases else 0.0
        )
        successful = [
            c["counterfactuals"][0]["risk_reduction"]
            for c in cases if c["counterfactuals"]
        ]
    ...
    ax[0].set_ylabel("Patients with ≥1 decision-flipping CF (%)")
    ax[0].set_title("Decision-flipping counterfactual success")
    ax[0].set_ylim(0, 105)
...
def write_case_csv(results: Dict[str, List[Dict]], path: Path):
    fields = [
        ...
        "decision_threshold", "predicted_class_before",
...
def prepare_output(base_out: Path):
    ...
    for name in ["xai_results_v3_2.json", "case_explanations.csv"]:
        p = base_out / name
        if p.exists():
            p.unlink()
    return plots
...
def main():
    ...
    base_out = Path(a.out) if a.out else Path(config.RESULTS) / "RQ3_xai_v3_2" / run_id
    ...
    for m, cases in results.items():
        n = len(cases)
        successful = [c for c in cases if c["counterfactuals"]]
        ...
        cf_summary[m] = {
            ...
            "decision_flip_success_pct": (100.0 * len(successful) / n) if n else 0.0,
    ...
    with open(base_out / "xai_results_v3_2.json", "w", encoding="utf-8") as f:
        json.dump(doc, f, indent=2, default=float)

    print(f"\nWROTE:\n  {base_out / 'xai_results_v3_2.json'}")
